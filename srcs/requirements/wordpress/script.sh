...
# ./wp-cli.phar core install --url=${WP_DOMAIN} --title=inception --admin_user=${WP_ADMIN_NAME} --admin_password=${WP_ADMIN_PASS} --admin_email=${WP_ADMIN_EMAIL} --allow-root
./wp-cli.phar user create ${WP_USER_NAME} ${WP_USER_EMAIL} --role=author --user_pass=${WP_USER_PASS} --allow-root
# ./wp-cli.phar option update home https://${WP_DOMAIN} --allow-root
# ./wp-cli.phar option update siteurl https://${WP_DOMAIN} --allow-root
php-fpm8.2 -F